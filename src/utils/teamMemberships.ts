...
export type TeamMemberRoleMap = Record<
  string,
  TeamMemberRoleInput | string | null | undefined
>;
...
function normalizeRoleInput(
  value: TeamMemberRoleInput | string | null | undefined,
...
export async function replaceTeamMemberships(params: {
  ...
  const targetEntries = Object.entries(params.memberRoles)
    .map(([userId, value]) => [userId, normalizeRoleInput(value)] as const)
    .filter((entry): entry is readonly [string, TeamMemberRoleInput] => !!entry[1]);
  ...
  for (const [userId, role] of targetEntries) {
...
export async function listActiveTeamMemberships(): Promise<TeamMembershipRosterEntry[]> {
  ...
  return ((data ?? []) as Array<{
    ...
    const role = [...(row.team_role_eligibilities ?? [])].sort(
      (a, b) => (a.sort_order ?? 0) - (b.sort_order ?? 0),
    )[0];
    return {
      teamId: row.team_id,
