-- Migration 089: complete the FX column set on billing_line_items.
--
-- Migration 081 added exchange_rate / base_currency / base_amount but skipped
-- original_currency and exchange_rate_date, leaving billing_line_items the
-- only multi-currency table that does not match the FX header pattern used
-- by invoices, collections, evouchers, and expenses. Reports that drill from
-- a header to lines have to special-case billing_line_items today; this
-- migration removes that asymmetry.
--
-- Backfill rule:
--   original_currency  := currency      (already populated from schema inception)
--   exchange_rate_date := created_at::DATE (best-effort proxy when unknown)
...
-- ---------------------------------------------------------------------------
-- Backfill existing rows from the legacy `currency` + `created_at` columns
-- ---------------------------------------------------------------------------
UPDATE billing_line_items
SET original_currency = COALESCE(currency, 'PHP')
WHERE original_currency IS NULL;

UPDATE billing_line_items
SET exchange_rate_date = COALESCE(exchange_rate_date, created_at::DATE, CURRENT_DATE)
WHERE exchange_rate_date IS NULL;

CREATE INDEX IF NOT EXISTS idx_billing_line_items_original_currency
  ON billing_line_items(original_currency);

-- ---------------------------------------------------------------------------
-- Redefine send_billing_items_to_booking() so virtual inserts persist the
-- two new FX columns. Body matches migration 085 with original_currency and
-- exchange_rate_date appended; everything else is unchanged.
-- ---------------------------------------------------------------------------
  ...
  LOOP
    ...
    IF v_is_virtual THEN
      ...
      v_rate_date := COALESCE(
        NULLIF(v_item->>'exchange_rate_date', '')::DATE,
        CURRENT_DATE
      );
