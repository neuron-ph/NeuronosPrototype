...
describe("access schema integrity", () => {
  ...
  it("door surfaces declare their own tab families (NEU-020 2.5 — no borrowed keys)", () => {
    expect(ACCESS_TAB_BY_MODULE_ID["bd_projects_info_tab"]).toBeDefined();
    expect(ACCESS_TAB_BY_MODULE_ID["bd_projects_billings_tab"]).toBeDefined();
    expect(ACCESS_TAB_BY_MODULE_ID["pricing_projects_info_tab"]).toBeDefined();
    expect(ACCESS_TAB_BY_MODULE_ID["bd_contracts_financial_overview_tab"]).toBeDefined();
    expect(ACCESS_NODE_BY_MODULE_ID["bd_projects"]?.containsModuleIds).toBeUndefined();
    expect(ACCESS_NODE_BY_MODULE_ID["pricing_projects"]?.containsModuleIds).toBeUndefined();
    expect(ACCESS_NODE_BY_MODULE_ID["bd_contracts"]?.containsModuleIds).toBeUndefined();
  });
