...
export function TruckingBookingDetails({ booking, onBack, onUpdate, currentUser, initialTab, highlightId }: TruckingBookingDetailsProps) {
  ...
  const canViewFinancials = can("ops_trucking_financials_tab", "view"); // Accounting-only per-booking financial dashboard
  const canViewInfo = can("ops_trucking_info_tab", "view");
  const canViewBillings = can("ops_trucking_billings_tab", "view");
  const canViewInvoices = can("ops_trucking_invoices_tab", "view");
  const canViewCollections = can("ops_trucking_collections_tab", "view");
  const canViewExpenses = can("ops_trucking_expenses_tab", "view");
  const canViewChrono = can("ops_trucking_chrono_tab", "view");
  const canViewCommentsTab = can("ops_trucking_comments_tab", "view"); // NEU-019 WG-15
  const canViewAttachments = can("ops_trucking_attachments_tab", "view");
  const canUploadAttachments = can("ops_trucking_attachments_tab", "create");
  const canDeleteAttachments = can("ops_trucking_attachments_tab", "delete");
  const canEditBooking = can("ops_trucking", "edit");
  const canCancelDeleteBooking = canEditBooking || can("ops_trucking", "delete");
    ...
    <div style={{ backgroundColor: "var(--theme-bg-surface)", display: "flex", flexDirection: "column", height: "100vh" }}>
      <div style={{ padding: "20px 48px", borderBottom: "1px solid var(--neuron-ui-border)", backgroundColor: "var(--theme-bg-surface)", display: "flex", justifyContent: "space-between", alignItems: "center", position: "relative", zIndex: 30 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <StatusSelector status={editedBooking.status} serviceType="Trucking" onUpdateStatus={handleStatusUpdate} readOnly={!canEditBooking} />
        </div>
      </div>
      ...
      <BookingCancelDeletePanel
        ...
        allowDelete={can("ops_trucking", "delete")} // NEU-019 WG-21
        onSuccess={(action) => {
          if (action === "deleted") {
            onBack();
          } else {
      ...
      <div style={{ flex: 1, overflow: "hidden", display: "flex", position: "relative", zIndex: 0 }}>
        <div style={{ flex: showTimeline ? "0 0 65%" : "1", overflow: "auto", transition: "flex 0.3s ease" }}>
          ...
          {activeTab === "booking-info" && canViewInfo && (
            <BookingInfoTab
              permissionDoor="ops_trucking_info_tab"
