...
import { Plus, Search, Shield, Briefcase, UserCheck, FileEdit, Clock, CheckCircle, Trash2, Archive } from "lucide-react";
...
export function MarineInsuranceBookings({ currentUser, pendingBookingId, initialTab, highlightId }: MarineInsuranceBookingsProps = {}) {
  ...
  const canViewAllTab        = can("ops_marine_insurance_all_tab", "view");
  const canViewMyTab         = can("ops_marine_insurance_my_tab", "view");
  ...
  const { scope, isLoaded: scopeLoaded } = useDataScope('bookings_marine_insurance');
  ...
  const { data: rawBookings = [], isLoading, refetch } = useQuery<MarineInsuranceBooking[]>({
    ...
    queryFn: async () => {
      ...
      return (data || []).map((row) => {
        const d = normalizeDetails(row.details || {}, "Marine Insurance");
        return {
          ...d,
          ...row,
          bookingId: row.id,
        ...
        <div style={{ padding: "32px 48px 24px 48px" }}>
          ...
          <div style={{ 
            ...
            >
              ...
              {getStatusOptions("Marine Insurance").map((status) => (
