...
export function CreateTruckingBookingPanel({
  ...
  useEffect(() => {
    ...
    fetchFullContract(detectedContractId).then(contract => {
      if (!contract?.rate_matrices) return;
      const destinations = extractContractDestinations(contract.rate_matrices);
      if (destinations.length > 0) {
        setFields({
          trucking_line_items: destinations.map(d => ({
            destination: d,
            truck_type: String(formState.truck_type ?? ""),
            quantity: "1",
          })),
        });
      }
    }).catch(console.error);
  }, [detectedContractId]); // eslint-disable-line react-hooks/exhaustive-deps
