...
export function MarineInsuranceBookingDetails({ booking, onBack, onUpdate, currentUser, initialTab, highlightId }: MarineInsuranceBookingDetailsProps) {
  ...
  const canViewFinancials = can("ops_marine_insurance_financials_tab", "view"); // Accounting-only per-booking financial dashboard
  ...
  return (
    <div style={{ backgroundColor: "var(--theme-bg-surface)", display: "flex", flexDirection: "column", height: "100vh" }}>
      ...
      <BookingCancelDeletePanel
        ...
        allowDelete={can("ops_marine_insurance", "delete")} // NEU-019 WG-21
        onSuccess={(action) => {
          if (action === "deleted") {
            onBack();
          } else {
      ...
      <div style={{ flex: 1, overflow: "hidden", display: "flex", position: "relative", zIndex: 0 }}>
        <div style={{ flex: showTimeline ? "0 0 65%" : "1", overflow: "auto", transition: "flex 0.3s ease" }}>
          ...
          {activeTab === "billings" && canViewBillings && <div className="flex flex-col bg-[var(--theme-bg-surface)] p-12 min-h-[600px]"><UnifiedBillingsTab items={bookingBillingItems} projectId={booking.projectNumber || ""} bookingId={booking.bookingId} onRefresh={financials.refresh} isLoading={financials.isLoading} pendingBillableCount={pendingBillableCount} extraActions={<BookingRateCardButton booking={booking} serviceType="Marine Insurance" existingBillingItems={bookingBillingItems} onRefresh={financials.refresh} permissionDoor="ops_marine_insurance_billings_tab" />} permissionDoor="ops_marine_insurance_billings_tab" /></div>}
