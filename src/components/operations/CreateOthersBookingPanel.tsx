...
export function CreateOthersBookingPanel({
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void (async () => {
      const preview = await peekNextBookingNumber("Others");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!can("ops_others", "create") && !can("ops_others", "edit")) return; // NEU-019 WG-32 backstop
    ...
    try {
      ...
      const bookingNumber = existingBookingNumber || (await generateBookingNumber("Others"));
      const { topLevel, details } = buildBookingPayload(formState, "Others");
      ...
      toast.success("Others booking created successfully");
      onSuccess?.(data);
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      console.error("CreateOthersBookingPanel saveDraft:", err);
  ...
  return (
    <BookingCreationPanel
      ...
      title={isEditingDraft ? "Edit Draft Others Booking" : "New Others Booking"}
      subtitle={
        isEditingDraft
          ? "Finish the draft and submit, or keep saving as draft"
          : source === "pricing"
          ? "Create a miscellaneous booking from project specifications"
          : "Create a new miscellaneous service booking"
      }
      ...
      {customerName && (
        <div style={{ marginBottom: "32px" }}>
          ...
          <div style={{ padding: "20px", backgroundColor: "var(--theme-bg-page)", border: "1px solid var(--neuron-ui-border)", borderRadius: "8px" }}>
            <ServiceRoleAssignmentForm
              key={`Others:${selectedCustomer.customerId ?? "no-customer"}`}
              customerId={selectedCustomer.customerId}
              serviceType="Others"
