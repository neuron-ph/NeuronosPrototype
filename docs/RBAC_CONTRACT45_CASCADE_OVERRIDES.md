...
3. ✅ **Flip enforcement to explicit-only** (migration 152 + `PermissionProvider` rewrite) — `current_user_effective_module_grant`, `current_user_visibility_scope`, `current_user_visibility_departments` now read `users.access_profile_id` → assigned profile (exact key; no override merge, no cascade, no role fallback); `PermissionProvider` reads that profile's grants directly. `access_profiles_select` is `using(true)` so the client can read its own profile. **Verified (dev):** all 60 assigned to active profiles; resolver spot-checks preserved (Cecil booking-create ✓, Jayson dept scope ✓, HR none, Accounting e-voucher ✓); diagnostics clean. **Behavioral note:** DB enforcement now matches what the UI showed (the prior client-cascade view), removing the pre-existing app/DB divergence — no one loses access; previously phantom-but-blocked actions become truly allowed. **→ Marcus smoke test pending** before Slices 4–5. (overrides + cascade now unused by enforcement but not yet dropped.)
4. ✅ **Editor cascade = UX-only** — `AccessProfiles.tsx` save now persists `resolveCascadedGrants(grants)` (fully explicit) instead of the sparse/derived set, so what's stored == what the editor showed == what the resolver enforces (no read-cascade). Cascade remains the in-editor convenience (display + tick-parent), but storage is explicit. Contained/borrowed tabs already render as toggleable rows when their parent is granted; explicit storage removes the prior cascade-vs-explicit-false confusion. Diagnostics clean. (Per-user grid editor in `AccessConfiguration` still writes overrides — removed in Slice 5.)
